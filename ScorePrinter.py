...
class ScorePrinter:
    ...
    def printAvg(self, showCount = True, last = None):
        ...
        avg_scores = getAvgScores(last)

        for metric_name in self.avg_scores.keys():
            avg_score = avg_scores[metric_name]
            if not isinstance(avg_score, list):
              print("{0}: {1:.2f}".format(metric_name, avg_score), end=' ')
            else: 
              print("\t".join(["{0}[{1}]: {2:.2f}".format(metric_name, i, float(x)) for i, x in enumerate(avg_score)]))
            
    def startEpoch(self, epoch):
        print(f"\n\nEpoch {epoch} started")

        self.scores = {metric[metric_name]: [] for metric_name, _ in self.metrics}
        self.count = 0
    ...
    def getAvgScores(self, last = None):
        ...
        for (metric_name, _) in self.metrics:
            ...
            if isinstance(avg_score, np.array):
                avg_scores[metric_name] = avg_score.list()
            else:
